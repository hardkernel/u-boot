...
static int calc_parts_list_len(int numparts)
{
	...
	/* per-partition additions;
	numparts starts at 1, so this should be correct */
...
static struct disk_part *allocate_disk_part(disk_partition_t *info, int partnum)
{
	...
	strncpy(newpart->gpt_part_info.uuid, (const char *)info->uuid,
		UUID_STR_LEN);
	/* UUID_STR_LEN is correct,
	as uuid[]'s length is UUID_STR_LEN+1 chars */
 ...
 */
static int get_gpt_info(struct block_dev_desc *dev_desc)
{
...
static int do_get_gpt_info(struct block_dev_desc *dev_desc)
{
	int ret;
 ...
 *
 */
static int set_gpt_info(struct block_dev_desc *dev_desc,
			const char *str_part,
			char **str_disk_guid,
	...
	for (i = 0; i < p_count; i++) {
		...
		if (val) {
			...
			if (strnlen(p, max_str_part) >=
				...
				printf("Wrong type guid format in part %d\n",
				       i);
				errno = -4;
				goto err;
			}
...
static int gpt_default(struct block_dev_desc *blk_dev_desc,
			 const char *str_part)
{
	int ret;
	char *str_disk_guid;
...
static int gpt_verify(struct block_dev_desc *blk_dev_desc, const char *str_part)
{
...
static int do_disk_guid(struct block_dev_desc *dev_desc, char * const namestr)
{
	int ret;
...
static int do_rename_gpt_parts(struct block_dev_desc *dev_desc, char *subcomm,
			       char *name1, char *name2)
{
	...
	if (!strcmp(subcomm, "swap")) {
		if ((strlen(name1) > PART_NAME_LEN) ||
		    (strlen(name2) > PART_NAME_LEN)) {
			printf("Names longer than %d are truncated.\n",
			       PART_NAME_LEN);
			ret = -EINVAL;
			goto out;
		}
		...
		ret = get_partition_info(dev_desc, partnum, new_partitions);
		if (ret < 0)
			goto out;
...
static int do_gpt(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if ((strcmp(argv[1], "write") == 0) && (argc == 5)) {
		printf("Writing GPT: ");
		ret = gpt_default(&mmc->block_dev, argv[4]);
	} else if ((strcmp(argv[1], "verify") == 0)) {
		ret = gpt_verify(&mmc->block_dev, argv[4]);
		printf("Verify GPT: ");
	} else if (strcmp(argv[1], "guid") == 0) {
		ret = do_disk_guid(&mmc->block_dev, argv[4]);
#ifdef CONFIG_CMD_GPT_RENAME
	} else if (strcmp(argv[1], "read") == 0) {
		ret = do_get_gpt_info(&mmc->block_dev);
	} else if ((strcmp(argv[1], "swap") == 0) ||
		   (strcmp(argv[1], "rename") == 0)) {
		ret = do_rename_gpt_parts(&mmc->block_dev, argv[1],
						 argv[4], argv[5]);
#endif
	} else {
		return CMD_RET_USAGE;
