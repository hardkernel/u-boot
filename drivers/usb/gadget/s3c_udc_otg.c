...
static struct amlogic_usb_config usb_config={
	0,//        USB_PHY_CLOCK_SEL_XTAL_DIV2,
        1, // no use if clock sel == xtal or xtal/2
	IO_USB_A_BASE,
        0,
        0, //set_vbus_power
};
...
int s3c_udc_probe(struct s3c_plat_otg_data *pdata)
{
	...
	for (i = 0; i < S3C_MAX_ENDPOINTS+1; i++) {
		...
		dcache_invalid_range((unsigned long) dev->dma_buf[i],
							 DMA_BUFFER_SIZE);
	}
