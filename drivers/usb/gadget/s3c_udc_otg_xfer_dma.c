...
static inline void s3c_udc_ep0_zlp(struct s3c_udc *dev)
{
	...
	dcache_flush_range((unsigned long) usb_ctrl_dma_addr, DMA_BUFFER_SIZE);
...
void s3c_udc_pre_setup(void)
{
	...
	dcache_invalid_range((unsigned long) usb_ctrl_dma_addr,
				DMA_BUFFER_SIZE);
...
static int setdma_rx(struct s3c_ep *ep, struct s3c_request *req)
{
	...
	dcache_invalid_range((unsigned long) ep->dev->dma_buf[ep_num], DMA_BUFFER_SIZE);
...
int setdma_tx(struct s3c_ep *ep, struct s3c_request *req)
{
	...
	dcache_flush_range((unsigned long) p , DMA_BUFFER_SIZE);
...
static void complete_rx(struct s3c_udc *dev, u8 ep_num)
{
	...
	dcache_invalid_range((unsigned long) p, DMA_BUFFER_SIZE);
...
int s3c_fifo_read(struct s3c_ep *ep, u32 *cp, int max)
{
	...
	dcache_invalid_range((unsigned long) ep->dev->dma_buf[ep_index(ep)], DMA_BUFFER_SIZE);
