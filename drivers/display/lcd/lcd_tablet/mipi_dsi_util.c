...
static void mipi_dsi_check_state(unsigned char reg, unsigned char cnt)
{
	int ret = 0, i;
...
static void mipi_dsi_phy_config(struct dsi_phy_s *dphy, unsigned int dsi_ui)
{
	...
	for (n = 0; n <= 0xff; n++) {
		...
		if (((temp * dsi_phy_config.hs_trail / 100) > t_req_min) &&
			...
			if (lcd_debug_print_flag) {
				LCDPR("t_req_min=%d, t_req_max=%d\n", t_req_min, t_req_max);
				LCDPR("t_ui=%d, hs_trail=%d, n=%d\n", t_ui, dsi_phy_config.hs_trail, n);
			}
			break;
		}
	}
	...
	for (n = 0; n <= 0xff; n++) {
		...
		if (((temp * dsi_phy_config.hs_prepare / 100) > t_req_min) &&
			((temp * dsi_phy_config.hs_prepare / 100) < t_req_max)) {
			if (lcd_debug_print_flag) {
				LCDPR("t_req_min=%d, t_req_max=%d\n", t_req_min, t_req_max);
				LCDPR("t_ui=%d, hs_prepare=%d, n=%d\n", t_ui, dsi_phy_config.hs_prepare, n);
			}
			break;
		}
	}
