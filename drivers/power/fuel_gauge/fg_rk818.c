...
{
	return i2c_reg_read(rk818_fg.p->hw.i2c.addr, reg);
}
...
{
	return rk818_bat_read(RK818_SOC_REG);
}
...
static void rk818_bat_enable_gauge(struct battery_info *di)
{
	...
	val = rk818_bat_read(RK818_TS_CTRL_REG);
	val |= GG_EN;
	rk818_bat_write(RK818_TS_CTRL_REG, val);
}
...
static int rk818_bat_get_vcalib0(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_VCALIB0_REGL) << 0;
	val |= rk818_bat_read(RK818_VCALIB0_REGH) << 8;
...
static int rk818_bat_get_vcalib1(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_VCALIB1_REGL) << 0;
	val |= rk818_bat_read(RK818_VCALIB1_REGH) << 8;
...
static int rk818_bat_get_ioffset(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_IOFFSET_REGL) << 0;
	val |= rk818_bat_read(RK818_IOFFSET_REGH) << 8;
...
static int rk818_bat_get_coffset(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_CAL_OFFSET_REGL) << 0;
	val |= rk818_bat_read(RK818_CAL_OFFSET_REGH) << 8;
...
static void rk818_bat_set_coffset(struct battery_info *di, int val)
{
	...
	buf = (val >> 0) & 0xff;
	rk818_bat_write(RK818_CAL_OFFSET_REGL, buf);
	buf = (val >> 8) & 0xff;
	rk818_bat_write(RK818_CAL_OFFSET_REGH, buf);
...
static void rk818_bat_init_coffset(struct battery_info *di)
{
	...
	di->poffset = rk818_bat_read(RK818_POFFSET_REG);
	if (!di->poffset)
...
static int rk818_bat_get_ocv_voltage(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_BAT_OCV_REGL) << 0;
	val |= rk818_bat_read(RK818_BAT_OCV_REGH) << 8;
...
static int rk818_bat_get_avg_current(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_BAT_CUR_AVG_REGL) << 0;
	val |= rk818_bat_read(RK818_BAT_CUR_AVG_REGH) << 8;
...
static int rk818_bat_get_avg_voltage(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_BAT_VOL_REGL) << 0;
	val |= rk818_bat_read(RK818_BAT_VOL_REGH) << 8;
	vol = di->voltage_k * val / 1000 + di->voltage_b;
...
static void rk818_bat_init_chrg_config(struct battery_info *di)
{
	...
	ggcon = rk818_bat_read(RK818_GGCON_REG);
	sup_sts = rk818_bat_read(RK818_SUP_STS_REG);
	usb_ctrl = rk818_bat_read(RK818_USB_CTRL_REG);
	thermal = rk818_bat_read(RK818_THERMAL_REG);
	chrg_ctrl2 = rk818_bat_read(RK818_CHRG_CTRL_REG2);
	chrg_ctrl3 = rk818_bat_read(RK818_CHRG_CTRL_REG3);
	...
	rk818_bat_write(RK818_GGCON_REG, ggcon);
	rk818_bat_write(RK818_SUP_STS_REG, sup_sts);
	rk818_bat_write(RK818_USB_CTRL_REG, usb_ctrl);
	rk818_bat_write(RK818_THERMAL_REG, thermal);
	rk818_bat_write(RK818_CHRG_CTRL_REG1, chrg_ctrl1);
	rk818_bat_write(RK818_CHRG_CTRL_REG2, chrg_ctrl2);
	rk818_bat_write(RK818_CHRG_CTRL_REG3, chrg_ctrl3);
}
...
static int rk818_bat_get_prev_cap(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_REMAIN_CAP_REG3) << 24;
	val |= rk818_bat_read(RK818_REMAIN_CAP_REG2) << 16;
	val |= rk818_bat_read(RK818_REMAIN_CAP_REG1) << 8;
	val |= rk818_bat_read(RK818_REMAIN_CAP_REG0) << 0;
...
static void rk818_bat_save_fcc(struct battery_info *di, u32 cap)
{
	...
	buf = (cap >> 24) & 0xff;
	rk818_bat_write(RK818_NEW_FCC_REG3, buf);
	buf = (cap >> 16) & 0xff;
	rk818_bat_write(RK818_NEW_FCC_REG2, buf);
	buf = (cap >> 8) & 0xff;
	rk818_bat_write(RK818_NEW_FCC_REG1, buf);
	buf = (cap >> 0) & 0xff;
	rk818_bat_write(RK818_NEW_FCC_REG0, buf);
}
...
static int rk818_bat_get_fcc(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_NEW_FCC_REG3) << 24;
	val |= rk818_bat_read(RK818_NEW_FCC_REG2) << 16;
	val |= rk818_bat_read(RK818_NEW_FCC_REG1) << 8;
	val |= rk818_bat_read(RK818_NEW_FCC_REG0) << 0;
...
static int rk818_bat_get_pwroff_min(struct battery_info *di)
{
	...
	cur = rk818_bat_read(RK818_NON_ACT_TIMER_CNT_REG);
	last = rk818_bat_read(RK818_NON_ACT_TIMER_CNT_SAVE_REG);
	rk818_bat_write(RK818_NON_ACT_TIMER_CNT_SAVE_REG, cur);
...
static int rk818_bat_get_coulomb_cap(struct battery_info *di)
{
	...
	val |= rk818_bat_read(RK818_GASCNT_REG3) << 24;
	val |= rk818_bat_read(RK818_GASCNT_REG2) << 16;
	val |= rk818_bat_read(RK818_GASCNT_REG1) << 8;
	val |= rk818_bat_read(RK818_GASCNT_REG0) << 0;
	val /= 2390;
...
static void rk818_bat_init_capacity(struct battery_info *di, u32 capacity)
{
	...
	buf = (cap >> 24) & 0xff;
	rk818_bat_write(RK818_GASCNT_CAL_REG3, buf);
	buf = (cap >> 16) & 0xff;
	rk818_bat_write(RK818_GASCNT_CAL_REG2, buf);
	buf = (cap >> 8) & 0xff;
	rk818_bat_write(RK818_GASCNT_CAL_REG1, buf);
	buf = (cap >> 0) & 0xff;
	rk818_bat_write(RK818_GASCNT_CAL_REG0, buf);
...
static int rk818_bat_get_usb_state(struct battery_info *di)
{
	...
	if ((rk818_bat_read(RK818_VB_MON_REG) & PLUG_IN_STS) != 0)
...
static void rk818_bat_clr_initialized_state(struct battery_info *di)
{
	...
	val = rk818_bat_read(RK818_MISC_MARK_REG);
	val &= ~FG_INIT;
	rk818_bat_write(RK818_MISC_MARK_REG, val);

	rk81x_fg_loader_init = false;
}

static bool rk818_bat_is_initialized(struct battery_info *di)
{
	return (rk818_bat_read(RK818_MISC_MARK_REG) & FG_INIT) ? true : false;
}
...
{
	return rk818_bat_read(RK818_HALT_CNT_REG);
}
...
static void rk818_bat_inc_halt_cnt(struct battery_info *di)
{
	...
	cnt = rk818_bat_read(RK818_HALT_CNT_REG);
	rk818_bat_write(RK818_HALT_CNT_REG, ++cnt);
}
...
static bool is_rk818_bat_first_poweron(struct battery_info *di)
{
	...
	buf = rk818_bat_read(RK818_GGSTS_REG);
	if (buf & BAT_CON) {
		buf &= ~BAT_CON;
...
static bool rk818_bat_ocv_sw_reset(struct battery_info *di)
{
	...
	buf = rk818_bat_read(RK818_MISC_MARK_REG);
	if (((buf & FG_RESET_LATE) && di->pwroff_min >= 30) ||
	    (buf & FG_RESET_NOW)) {
		buf &= ~FG_RESET_LATE;
		buf &= ~FG_RESET_NOW;
		rk818_bat_write(RK818_MISC_MARK_REG, buf);
...
static void rk818_bat_init_ts1(struct battery_info *di)
{
	u8 buf;
...
{
	return (rk818_bat_read(RK818_SUP_STS_REG) & BAT_EXS) ? true : false;
}
...
static int rk818_bat_check(struct pmic *p, struct pmic *bat)
{
...
static u8 rk818_bat_get_chrg_status(struct battery_info *di)
{
	...
	status = rk818_bat_read(RK818_SUP_STS_REG) & BAT_STATUS_MSK;
	switch (status) {
	case CHARGE_OFF:
		DBG("CHARGE-OFF...\n");
		break;
...
static void rk818_bat_debug_info(struct battery_info *di)
{
	...
	if (!dbg_enable)
		return;
	ggcon = rk818_bat_read(RK818_GGCON_REG);
	ggsts = rk818_bat_read(RK818_GGSTS_REG);
	sup_sts = rk818_bat_read(RK818_SUP_STS_REG);
	usb_ctrl = rk818_bat_read(RK818_USB_CTRL_REG);
	thermal = rk818_bat_read(RK818_THERMAL_REG);
	vb_mod = rk818_bat_read(RK818_VB_MON_REG);
	misc = rk818_bat_read(RK818_MISC_MARK_REG);
	rtc = rk818_bat_read(RK818_SECONDS_REG);
	chrg_ctrl1 = rk818_bat_read(RK818_CHRG_CTRL_REG1);
	chrg_ctrl2 = rk818_bat_read(RK818_CHRG_CTRL_REG2);
	chrg_ctrl3 = rk818_bat_read(RK818_CHRG_CTRL_REG3);
...
static void rk818_bat_set_term_mode(struct battery_info *di, int mode)
{
	...
	buf = rk818_bat_read(RK818_CHRG_CTRL_REG3);
	buf &= ~CHRG_TERM_SIG_MSK;
	buf |= mode;
	rk818_bat_write(RK818_CHRG_CTRL_REG3, buf);
...
static bool rk818_bat_adc_calib(struct battery_info *di)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (abs(adc) < ADC_CALIB_THRESHOLD) {
			...
			rk818_bat_write(RK818_POFFSET_REG, di->poffset);
 ...
 */
static int rk818_bat_get_ntc_res(struct battery_info *di)
{
	...
	if (!adc1_update) {
		...
		/* read adc adc1 */
		ua1 = di->ntc_uA;
		adc1 |= rk818_bat_read(RK818_TS1_ADC_REGL) << 0;
		adc1 |= rk818_bat_read(RK818_TS1_ADC_REGH) << 8;
	...
	/* read adc adc2 */
	adc2 |= rk818_bat_read(RK818_TS1_ADC_REGL) << 0;
	adc2 |= rk818_bat_read(RK818_TS1_ADC_REGH) << 8;
...
static int rk818_bat_update_temperature(struct battery_info *di)
{
...
static int rk818_bat_update(struct pmic *p, struct pmic *bat)
{
	struct battery *battery = bat->pbat->bat;
	...
	/* set charge current */
	rk818_fg.di.state_of_chrg =
		rk818_bat_get_charger_type(&rk818_fg.di, bat);
	rk818_bat_charger_setting(&rk818_fg.di, rk818_fg.di.state_of_chrg);
	...
	battery->state_of_chrg = rk818_fg.di.state_of_chrg;
...
static struct power_fg fg_ops = {
	.fg_battery_check = rk818_bat_check,
...
static int rk818_bat_set_info(struct battery_info *di)
{
	...
	len = sizeof(rk818_ocv_table);
	di->ocv_table = calloc(len*4, 1);
	...
	di->ocv_size = len;
	memcpy(di->ocv_table, rk818_ocv_table, sizeof(rk818_ocv_table));

	printf("ocv table size = %d\n",len);

	di->design_cap		= 4000; /* "design_capacity" */
	di->qmax		= 4020; /* "design_qmax" */
	di->dts_vol_sel		= 4250; /* "max_chrg_voltage" */
	di->dts_cur_input 	= 1500; /* "max_input_current" */
	di->dts_cur_sel 	= 2000; /* "max_chrg_current" */
...
//	di->ts2_vol_multi 	= 0; /* "ts2_vol_multi" */
	...
	di->dc_det.name = "dc_det_gpio";
	di->dc_det.gpio = GPIOAO_8;
	di->dc_det.flags = 0;
	gpio_request(di->dc_det.gpio, di->dc_det.name);
