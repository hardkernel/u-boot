...
int rk_board_late_init(void)
{
	...
	for (; n < 32 * 4; n += 32) {
		...
		if (is_valid_ethaddr((unsigned char *)&node)) {
...
#if defined(CONFIG_TARGET_ODROID_M2)
	int devnum = 0;	// MMC device number to access
	struct mmc *mmc = find_mmc_device(devnum);
	if (mmc) {
		// Access the first 'boot' partition and read first block
		// that stores an device identification number (UUID type 1).
		int ret = blk_select_hwpart_devnum(IF_TYPE_MMC, devnum, 2);
		if (!ret) {
			blk_dread(mmc_get_blk_desc(mmc), 8191, 1, (void*)load_addr);
