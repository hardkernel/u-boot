/*
 * ODROID movi command to read/write partition data
 *
...
static int do_movi(cmd_tbl_t* cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (isstring(argv[2])) {
		...
		ptn = disk_flash_find_ptn(argv[2]);
		if (ptn == NULL) {
...
U_BOOT_CMD(
		movi,			7,		0,		do_movi,
		"Read/write command from/to SD/MMC for ODROID-N1 board",
