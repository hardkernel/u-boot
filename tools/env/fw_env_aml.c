...
#define ENVCUROFFSET(i)  envdevices[(i)].envcuroff
#define ENVNEXTOFFSET(i) envdevices[(i)].envnextoff
#define ENVSIZE(i)    envdevices[(i)].env_size
#define DEVESIZE(i)   envdevices[(i)].erase_size
#define DEVPAGESIZE(i) envdevices[(i)].env_pagesize
...
static int env_offset_init(void);
...
{
	int rc;
	int pagesize, env_size, datasize, flagoff;
	char  *data=NULL;
	...
	// AML: one block: env + onepage start with 0xaa55
	if(DEVTYPE(dev_target) == MTD_NANDFLASH){
		pagesize = DEVPAGESIZE(dev_target);
		env_size = ENVSIZE(dev_target);		
		if (pagesize > env_size){				
				datasize = pagesize+pagesize;
				flagoff = pagesize;				
			}
		else{
				datasize = env_size+pagesize;
				flagoff = env_size;	
			}
		data = malloc (datasize);
		if (!data) {
			fprintf (stderr,	 "Cannot malloc %u bytes: %s\n",	 datasize, strerror (errno));
			return -1;
		}
		memset(data, 0xff, datasize);
		memcpy(data, (char*)environment.image, env_size);
		data[flagoff] = 0xaa;
		data[flagoff+1] = 0x55;
		
		rc = flash_write_buf (dev_target, fd_target, data,  datasize, ENVNEXTOFFSET (dev_target),  DEVTYPE(dev_target));
		...
		rc = flash_write_buf (dev_target, fd_target, environment.image,
				      CONFIG_ENV_SIZE, ENVNEXTOFFSET (dev_target),
				      DEVTYPE(dev_target));
		if (rc < 0)
			return rc;
	}
...
//=====================================================================================
static int env_offset_init_sub(int dev)
{
	int fd, i, error;
	struct mtd_info_user mtdinfo;
	int rc;
	int pagesize, blocksize, env_size, len;
	unsigned int addr = 0;
	unsigned offset = DEVOFFSET(dev);
	char env_flag = 0;
	loff_t badtest;
	char env_status[10];
	int nextidx;	
	...
	if(mtdinfo.type == MTD_NANDFLASH){
		...
		for(i=0; i<ENVSECTORS(dev); i++){  //scan block, find valid env block
			...
			env_status[i] = -1;			
			badtest = addr;
			if(flash_bad_block(fd, mtdinfo.type, &badtest))
				continue;
			...
			error = read (fd, temp_buf, len);				
			if (error == len) {
				if((temp_buf[0] == 0xaa)&&(temp_buf[1] == 0x55)){					
					env_flag ++;
...
{
	if(env_offset_init_sub(0))
			return -1;
			
	if(HaveRedundEnv)
		if(env_offset_init_sub(1))
			return -1;
...
static int parse_config ()
{
	...
	ENVCUROFFSET(0)=DEVICE1_OFFSET;
	ENVNEXTOFFSET(0) = DEVICE1_OFFSET;
#ifdef HAVE_REDUND
	...
	ENVCUROFFSET(1)=DEVICE2_OFFSET;
	ENVNEXTOFFSET(1) = DEVICE2_OFFSET;
	HaveRedundEnv = 1;
#endif
#endif
