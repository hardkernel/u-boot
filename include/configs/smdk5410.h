...
#if 0
    #define CONFIG_PMIC_CONTROL             /* PMIC Setting : BUCK, LDO */
#endif
#define CONFIG_ERASE_UBOOT_ENV          /* Bootloader update : erase env */

#if defined(CONFIG_BOARD_HARDKERNEL)
    #define CONFIG_TOSHIBA_EMMC441      /* eMMC4.41 Reset Enable */
    #define ON                  1
    #define OFF                 0
    
    #define CONFIG_LED_CONTROL
    #if defined(CONFIG_LED_CONTROL)
        #define LED_GREEN(x)        GPB2DAT =  (x ? (GPB2DAT | 0x02) : (GPB2DAT & 0xFD))
        #define LED_BLUE(x)         GPB2DAT =  (x ? (GPB2DAT | 0x04) : (GPB2DAT & 0xFB))
        #define LED_RED(x)          GPX2DAT =  (x ? (GPX2DAT | 0x08) : (GPX2DAT & 0xF7))
    #endif
...
/* Bootloader Recovery */
#if defined(CONFIG_MACH_UNIVERSAL5410) || defined(CONFIG_MACH_ASB5410) || defined(CONFIG_BOARD_HARDKERNEL)
...
#if defined(CONFIG_BOARD_HARDKERNEL)
    #define CONFIG_MACH_TYPE		MACH_TYPE_ODROIDXU
#else
 ...
 * USBD 3.0 SFR
 */
#ifdef CONFIG_EXYNOS_USBD3_CH0
    #define USBDEVICE3_LINK_BASE		0x12000000
    #define USBDEVICE3_PHYCTRL_BASE		0x12100000
#else
    #define USBDEVICE3_LINK_BASE		0x12400000
    #define USBDEVICE3_PHYCTRL_BASE		0x12500000
#endif
...
#define CONFIG_SPL
    ...
    #define CONFIG_UBOOT_COPY_SD2EMMC   \
		...
		"mmc write 1 0x40008000 0x4CF 0x20;"

    #define CONFIG_UBOOT_COPY_EMMC2SD   \
		"emmc open 0;movi r z f 0 40000000;emmc close 0;movi w f 1 40000000;"	\
		"emmc open 0;movi r z b 0 40000000;emmc close 0;movi w b 1 40000000;"	\
		"emmc open 0;movi r z u 0 40000000;emmc close 0;movi w u 1 40000000;"	\
		"emmc open 0;movi r z t 0 40000000;emmc close 0;movi w t 1 40000000;"	\
		"mmc write 1 0x40008000 0x4CF 0x20;"
#else
...
#if defined(CONFIG_BOARD_HARDKERNEL)
    #define CONFIG_IDENT_STRING		" for Exynos5410"
#else
